...
def run(procedure, run_mode, image, n_drawables, drawables, args, data):
    ...
    class DSGDialog(GimpUi.Dialog):
        def __init__(self):
            ...
            Gimp.get_pdb().run_procedure('gimp_image_undo_group_start', [GObject.Value(Gimp.Image, image)])
        ...
        def new_grammar( self ):
            ...
            def saveCurrentImg(img):
                org_img = img.duplicate()
                org_layer = org_img.merge_visible_layers( Gimp.MergeType.CLIP_TO_IMAGE )
                org_img_path = f'image.png'
                save_image(org_img, org_layer, org_img_path )
                deleteDuplicateImg(org_img)
                Gimp.get_pdb().run_procedure('gimp-displays-flush', [ ])
                return org_img_path
            def deleteDuplicateImg(img):
                Gimp.get_pdb().run_procedure('gimp-image-delete', [GObject.Value( Gimp.Image, img )])
            def saveSelectionImg(img):
                selection_img = img.duplicate()
                ...
                selection_img_path = f'selection.png'
                save_image( selection_img, selection_layer, selection_img_path )
                deleteDuplicateImg(selection_img)
                Gimp.get_pdb().run_procedure('gimp-displays-flush', [ ])
                return selection_img_path
              
            #org_img.delete()          
            # save selection image

            active_image=get_image()
            org_img_path=saveCurrentImg(active_image)
            selection_img_path= saveSelectionImg(active_image)
            ...
            file = 'makaniski.png' 
            layer = Gimp.get_pdb().run_procedure('gimp-file-load-layer', [ Gimp.RunMode.INTERACTIVE, active_image,  GObject.Value(Gio.File, Gio.File.new_for_path(file))])
            #active_image.insert_layer(layer.index(1), None, -1)
            sg.new_grammar1(get_image(), bboxes, layer.index(1))
            Gimp.get_pdb().run_procedure('gimp-displays-flush', [ ])
